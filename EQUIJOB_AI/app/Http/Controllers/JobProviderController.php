...
class JobProviderController extends Controller
{
    public function ViewJobProviderDashboard()
    {
        ...
        $jobPostingCount = JobPosting::where('status', 'For Posting')->count();
        $jobApplicationCount = JobPosting::count();
        $jobApplicantInterviewCount = JobApplication::where('status', 'For Interview')->count();

        $response = response()->view('users.job-provider.job_provider_dashboard', compact('user', 'notifications', 'unreadNotifications', 'jobPostingCount', 'jobApplicationCount', 'jobApplicantInterviewCount'));
