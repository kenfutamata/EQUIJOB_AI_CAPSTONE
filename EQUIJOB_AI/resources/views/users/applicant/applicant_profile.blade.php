...
<body class="min-h-screen bg-gray-100 text-gray-800 font-sans antialiased">
    ...
    <div class="flex flex-col ml-0 lg:ml-[234px] ">
        ...
        <div class="sticky top-16 z-10 bg-white shadow-sm flex items-center justify-between px-6 py-4 flex-shrink-0 h-14">
            <h1 class="font-audiowide text-3xl md:text-4xl text-gray-800">
                <span class="text-[#25324B]">Applicant </span><span class="text-[#26A4FF]">Profile</span>
            </h1>
        </div>
        ...
        <main class="p-6 md:p-10">
            <div class="max-w-5xl mx-auto">
                <div class="flex flex-col items-center mb-8">
                    ...
                    ? $user->profilePicture
                    : ($user->profilePicture ? "https://zlusioxytbqhxohsfvyr.supabase.co/storage/v1/object/public/equijob_storage/{$user->profilePicture}" : null);
                    @endphp
                            ...
                            <div> <label class="block text-sm font-medium text-gray-700 mb-2">Skills Certificates</label>
                                ...
                                <div class="space-y-2 border rounded-md p-3 mb-4 bg-gray-50">
                                    ...
                                    <div id="cert-{{ $loop->index }}" class="flex items-center justify-between text-sm">
                                        <a href="{{ $certificateUrl }}" target="_blank" rel="noopener noreferrer" class="flex items-center text-indigo-600 hover:underline truncate pr-4"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 6a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd" /></svg><span class="truncate">{{ basename($certificateUrl) }}</span></a>
                                    </div>
                                    @endforeach
                                </div>
                                @else
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="province" name="provinceId" class="w-full border rounded-md px-4 py-2 text-sm">
                            ...
                            <option value="{{ $province->id }}" {{ old('provinceId', $user->provinceId) == $province->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        @if($user->certificates && count($user->certificates) > 0)
                        <p class="text-xs text-gray-600 mb-2">Manage currently uploaded files:</p>
                        <div class="space-y-2 border rounded-md p-3 mb-4 bg-gray-50">
                            @foreach($user->certificates as $certificateUrl)
                            <div id="existing-cert-{{ $loop->index }}" class="flex items-center justify-between text-sm transition-opacity duration-300">
                                <a href="{{ $certificateUrl }}" target="_blank" rel="noopener noreferrer" class="flex items-center text-indigo-600 hover:underline truncate pr-4 transition-colors duration-300">
                                    ...
                                    <span class="truncate">{{ basename($certificateUrl) }}</span>
                                </a>
                                <button type="button" class="remove-existing-cert-btn ml-4 text-red-600 hover:text-red-800 flex-shrink-0 text-lg font-bold" data-url="{{ $certificateUrl }}" data-target-id="existing-cert-{{ $loop->index }}">&times;</button>
                            </div>
                            @endforeach
                        </div>
                        @else
                        ...
                        <div id="upload-area" class="flex justify-center w-full px-6 py-10 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-indigo-500 bg-gray-50">
                            <div class="space-y-1 text-center"><svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>
                                <div class="flex text-sm text-gray-600"><p class="pl-1">Click to upload or drag and drop</p></div>
                                <p class="text-xs text-gray-500">PNG, JPG, PDF up to 4MB</p>
                            </div>
                        </div>
                        <input type="file" id="certificatesInput" name="certificates[]" multiple class="hidden" accept="image/*,application/pdf" />
    ...
    {{-- ================================================================ --}}
    {{-- === NEW AND IMPROVED SCRIPT SECTION ============================ --}}
    {{-- ================================================================ --}}
    <script>
        function closeModal() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchAndPopulateCities() {
                ...
                const currentCityId = "{{ $user->cityId ?? '' }}";
            ...
            provinceSelect.addEventListener('change', fetchAndPopulateCities);


            // --- New Certificate Upload Logic ---
            function updateFileInputAndPreview() {
                previewContainer.innerHTML = '';
                if (selectedFiles.length > 0) {
                    const header = document.createElement('h4');
                    header.className = 'text-sm font-medium text-gray-700';
                    header.textContent = `New Files to Upload (${selectedFiles.length}):`;
                    ...
                    selectedFiles.forEach((file, index) => {
                        ...
                        filePreview.innerHTML = `
                            ...
                            <button type="button" data-index="${index}" class="remove-new-file-btn ml-4 text-red-600 hover:text-red-800 flex-shrink-0 font-bold text-lg">&times;</button>
                        `;
            ...
            if (uploadArea) {
                ...
                ['dragenter', 'dragover'].forEach(eventName => uploadArea.addEventListener(eventName, () => uploadArea.classList.add('border-indigo-500', 'bg-indigo-50')));
                ['dragleave', 'drop'].forEach(eventName => uploadArea.addEventListener(eventName, () => uploadArea.classList.remove('border-indigo-500', 'bg-indigo-50')));
            ...
            // --- Existing Certificate Deletion Logic (with Undo) ---
            document.querySelectorAll('.remove-existing-cert-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const linkElement = certificateElement.querySelector('a');
                    
                    const isMarkedForDeletion = certificateElement.classList.contains('opacity-50');

                    if (isMarkedForDeletion) {
                        // --- UNDO a deletion ---
                        certificateElement.classList.remove('opacity-50');
                        linkElement.classList.remove('line-through', 'text-gray-500', 'pointer-events-none');
                        linkElement.classList.add('text-indigo-600');

                        this.innerHTML = '&times;';
                        this.classList.remove('text-blue-600', 'hover:text-blue-800', 'text-xs', 'font-semibold');
                        this.classList.add('text-red-600', 'hover:text-red-800', 'text-lg', 'font-bold');
                        
                        const inputToDelete = deletionsContainer.querySelector(`input[value="${certificateUrl}"]`);
                        if (inputToDelete) {
                            inputToDelete.remove();
                        }
                    } else {
                        // --- MARK a file for deletion ---
                        certificateElement.classList.add('opacity-50');
                        linkElement.classList.add('line-through', 'text-gray-500', 'pointer-events-none');
                        linkElement.classList.remove('text-indigo-600');
                        
                        this.innerHTML = 'Undo';
                        this.classList.add('text-blue-600', 'hover:text-blue-800', 'text-xs', 'font-semibold');
                        this.classList.remove('text-red-600', 'hover:text-red-800', 'text-lg', 'font-bold');
